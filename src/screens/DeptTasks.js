...
class DeptTasks extends Component {
  constructor(props) {
    super(props);
  ...
  async getResults(strEmployee, strDBName, searchText) {
    ...
    if (databool === true) {
      try {
        var url = API_LINK_PUBLIC +  "Home/GetAllTasks?type=3&strEmployee=" + strEmployee + "&dbName=" + strDBName + "&searchText=" + searchText + "&searchDate=" + this.state.year + "/" + this.state.month + "/" + this.state.day;
        //console.warn(url);
        ...
        <Grid style={{ flex: 1, margin: 2 }}>
          ...
          {this.state.MangerResults.length > 0 // condition
            ...
            renderRow={data =>
              <ListItem
                button
                onPress={() => this.props.navigation.navigate("TaskDetails", { tabName: "تفاصيل المهمة", SentResults: data, bDeleteBtn: false })}
              >
                <Left>
                  <Text>
