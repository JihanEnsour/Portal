...
class AchivedTasks extends Component {
  constructor(props) {
    super(props);
  ...
  async getResults(strEmployee, strDBName, searchText) {
    ...
    if (databool === true) {
      try {
        var url = API_LINK_PUBLIC +  "Home/GetAllTasks?type=4&strEmployee=" + strEmployee + "&dbName=" + strDBName + "&searchText=" + searchText + "&searchDate=" + this.state.year + "/" + this.state.month + "/" + this.state.day;
        //console.warn(url);
