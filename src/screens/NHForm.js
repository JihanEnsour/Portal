...
class NHForm extends Component {
  ...
  fillGottenData() {
    AsyncStorage.getItem("UserLoggedData").then(value => {
      ...
      this.getResults(data[0].strEmployee, data[0].strDBName, this.state.searchText);
    });
  }
  async getResults(strEmployee, strDBName, searchText) {
    ...
    if (yyyy !== "0" && MM !== "0" && dd === "0") {
      databool = false;
      Toast.show({ text: "الرجاء تحديد اليوم", buttonText: "Ok", type: "warn" });
    } else if (yyyy !== "0" && MM === "0" && dd !== "0") {
      databool = false;
      Toast.show({ text: "الرجاء تحديد الشهر", buttonText: "Ok", type: "warn" });
    } else if (yyyy === "0" && MM !== "0" && dd !== "0") {
      databool = false;
      Toast.show({ text: "الرجاء تحديد السنة", buttonText: "Ok", type: "warn" });
    } else if ((yyyy !== "0" && MM === "0" && dd === "0") || (yyyy === "0" && MM !== "0" && dd === "0") || (yyyy === "0" && MM === "0" && dd !== "0")) {
      databool = false;
      Toast.show({ text: "الرجاء تحديد السنة-الشهر-اليوم", buttonText: "Ok", type: "warn" });
    }

    if (databool === true) {
      try {
        var url = JsonData.API_LINK_PUBLIC +  "Home/GetAllTasks?type=2&strEmployee=" + strEmployee + "&dbName=" + strDBName + "&searchText=" + searchText + "&searchDate=" + this.state.year + "/" + this.state.month + "/" + this.state.day;
        //console.warn(url);
  ...
  render() {
    return (
      <Container style={styles.container}>
        <Header>
          <Left>
            <Button transparent onPress={() => this.props.navigation.navigate("Tasks", { tabName: "المهمات" })}>
        ...
        <Content padder>
          <Grid>
            <Row style={[styles.row, { alignSelf: "center" }]}>
              <Item regular style={{ width: 110 }}>
                <Label>يوم</Label>
                <Picker
                  mode="dropdown"
                  iosHeader="يوم"
            ...
            <Row style={[styles.row, { marginTop: 10, alignSelf: "center" }]}>
              <Item regular style={{ width: 350 }}>
                <Icon active name="search" />
                <Input style={{ textAlign: "right" }} onChangeText={text => this.updateValues(text, "textToSearch")} placeholder="بحث..." />
                <Button style={{ marginTop: 2, marginLeft: 3 }} onPress={() => this.fillGottenData()} >
                  <Text>ابحث</Text>
                </Button>
              </Item>
            </Row>
          ...
          {this.state.SentResults.length > 0 // condition
            ...
            <List
              ...
              renderRow={data =>
                <ListItem
                  button
                  onPress={() => this.props.navigation.navigate("TaskDetails", { tabName: "تفاصيل المهمة", SentResults: data })}
                >
                  <Left>
                    <Text>
                      {"مستلم المهمة: " + data.ReaciverName}
                    </Text>
                  </Left>
                  <Right>
