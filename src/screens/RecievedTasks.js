...
class RecievedTasks extends Component {
  constructor(props) {
    super(props);
  ...
  async getRecievedResults(strEmployee, strDBName, searchText) {
    var databool = true;
    ...
    //console.warn(databool);
    if (databool === true) {
      try {
        var url =
          API_LINK_PUBLIC +  "Home/GetAllTasks?type=1&strEmployee=" +
          strEmployee +
          "&dbName=" +
          strDBName +
          "&searchText=" +
            ...
            <List
              ...
              renderRow={(data) => (
                <ListItem
                  button
                  onPress={() =>
                    this.props.navigation.navigate("RecievedTaskDetails", {
                      tabName: "تفاصيل المهمة",
                      RecievedResults: data,
                    })
                  }
                >
                  <Left>
                    <Text>{"مرسل المهمة: " + data.SenderName}</Text>
                  </Left>
                  <Right>
